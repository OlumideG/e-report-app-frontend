...
function UserDashboard({ setAuth }) {
  ...
  useEffect(() => {
    ...
    fetch(
      `https://ancient-citadel-22859.herokuapp.com/userinfo/username/${user}`
    )
  ...
  const forwardedReports = () => {
    ...
    fetch(
      `https://ancient-citadel-22859.herokuapp.com/dashboard/home/forwarded/${user}`
    )
      .then((res) => res.json())
      .then((result) => {
        setReports(result);
  ...
  const pendingReports = () => {
    ...
    fetch(
      `https://ancient-citadel-22859.herokuapp.com/dashboard/home/pending/${user}`
    )
      .then((res) => res.json())
      .then((result) => {
        setReports(result);
      ...
      <div className="dashboard-container">
        <div className="dashboard-handler">
          ...
          <div className="dashboard-content">
            {name.map((info, index) => (
              <User key={index} index={index} info={info} />
            ))}
            ...
            <div className="part-2">
              <div className="history-container">
                <button className="history-text" onClick={historyReportsButton}>
                  History
                </button>
              </div>
            ...
            <div className={!showHistory ? "hide" : "part-3"}>
              <div className="all-report">
                <button className="all-report-text" onClick={allReports}>
                  All
                </button>
              </div>

              <div className="forwarded-cases">
                <button className="forwarded-text" onClick={forwardedReports}>
                  Forwarded
                </button>
              </div>

              <div className="pending">
                <button className="pending-text" onClick={pendingReports}>
                  Pending
                </button>
              </div>
            </div>
