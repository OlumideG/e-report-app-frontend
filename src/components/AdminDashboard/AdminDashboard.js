...
function AdminDashboard({ setAdminAuth, index, info }) {
  ...
  const getProfile = async () => {
    try {
      const res = await fetch("https://ancient-citadel-22859.herokuapp.com/dashboard/", {
        method: "POST",
  ...
  const deleteReport = (x, info) => {
    ...
    fetch(`https://ancient-citadel-22859.herokuapp.com/dashboard/home/${reportNumber}`, {
  ...
  const forwardReport = (x, info) => {
    ...
    // console.log(x)
    // console.log(reportNumber)
    fetch(`https://ancient-citadel-22859.herokuapp.com/dashboard/forwarded/${reportNumber}`, {
      headers: { "Content-Type": "application/json; charset=utf-8" },
      method: 'PATCH',
      mode: 'cors',
    })
  ...
  const privateReportsButton =()=>{
    ...
    fetch(`https://ancient-citadel-22859.herokuapp.com/dashboard/admin/home/private`)
    .then(res => res.json())
    .then(result => {
      setReports(result)
